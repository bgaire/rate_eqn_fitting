...
for a = 1:length(in_times)
    if in_times(a) <= in_off_time
        out_curve_states(:,a) = expm(in_tm_nat*in_times(a))*steady_state;
        out_curve(a) = in_scale*in_vis_nat*out_curve_states(:,a);
    else
        out_curve_states(:,a) = expm((in_tm_nat+in_tm_drive*in_drive_level)*(in_times(a)-in_off_time))*off_exp*steady_state;
